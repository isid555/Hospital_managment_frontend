...
const BASE_URL = `${backendURL}/prescription`;
...
export default function usePrescription() {
    ...
    const fetchPrescriptionsByUserAndDoctor = async (userId, doctorId) => {
        const res = await axios.get(
            `${backendURL}/prescription/by-user-doctor?userId=${userId}&doctorId=${doctorId}`,
            {
                headers: {
    ...
    const createPrescription = async (data) => {
        try {
            const res = await axios.post(`${backendURL}/prescription/create`, data, {
                headers: { "auth-token": token }
            });
